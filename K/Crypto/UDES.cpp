...
#ifdef LOGGING
		printf ("K[%02d] = %06.6x%06.6x\n", index_j, outSubKeys[index_j].fHigh, outSubKeys[index_j].fLow);
#endif
	} // for (index_j = 0; index_j < 16; index_j++)
}
...
#ifdef LOGGING
  printf ("Perm. data = %08x %08x\n", permutedDataLeft, permutedDataRight);
#endif
...
#ifdef LOGGING
		printf ("K[%02d] = %06.6x%06.6x\n", index_i, inSubKeys[index_i].fHigh, inSubKeys[index_i].fLow);
    printf ("e  = %06.6x %06.6x\n", ExpandedHigh, ExpandedLow);
#endif
...
#ifdef LOGGING
        printf ("%02d ", kSBoxes[index_j][index_m][ temp >> 1 ]);
#endif        
...
#ifdef LOGGING
    printf ("L[%02d] = %08x R[%02d] = %08x\n", index_i, permutedDataLeft, index_i, permutedDataRight);
#endif    
	} // for (index_i = 0; index_i < 16; index_i++)
...
#ifdef LOGGING
    		tmpKeyHi = UTInt64::GetHi( theKey );
    		tmpKeyLo = UTInt64::GetLo( theKey );
		    printf( "theKey2 = %.8X%.8X\n",
					(unsigned int) tmpKeyHi,
					(unsigned int) tmpKeyLo );
#endif
