...
// ***** END LICENSE BLOCK *****
// ===========
// $Id: TThread.cp,v 1.3 2006/07/07 08:19:16 pguyot Exp $
// ===========
...
TThread::Suspend( void )
{
	...
	if (mState == kRunning)
	{
		...
		assert( err == 0 );
		(void) err;
	} else if ((mState == kSuspended) || (mState == kSleeping)) {
		mSuspendCount++;
	}
...
TThread::Resume( void )
{
	...
	{
		mSuspendCount--;
		if ((mState == kSuspended) && (mSuspendCount == 0))
		{
			mState = kRunning;
	...
	if (mWakeCount > 0)
	{
		...
		if (!forever)
		{
			...
			if (!forever)
			{
				...
				// Traduction en timespec et ajout du dŽlai.		
			...
			theResult = !mCondVar.TimedWait(&mMutex, &theTimeToWake);
		} else {
			theResult = false;
