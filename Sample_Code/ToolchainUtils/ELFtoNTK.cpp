    ...
    while (true) {
        ...
        if (str[0] != ';' && str[0] != '\n' && str[0] != 0) {
            ...
            if (firstLine) {
                ...
                entryPoint->Set("numArgs", TDCLNSRef::MakeInt(atoi((const char*) (const char*) crsr)));
...
int main( int argc, char** argv )
{
    ...
    try {
        ...
        TDCLPlatformFile expFile( &theFilesIntf, argv[2] );
        expFile.Open(true);

        TELFFile elfFile(&theFilesIntf, argv[5]);
        elfFile.Open(true);

        TDCLPlatformFile outputFile( &theFilesIntf, argv[4] );
        outputFile.Create();
