...
// Pour l'instant on dŽvermine ce truc.
#ifndef NDEBUG
#undef KDEBUG_ENABLED
#define KDEBUG_ENABLED 1
#endif
...
TDCLSocket::DoStopListening( void )
{
	if (mPublicPairMember >= 0)
	{
		// On Žcrit dans la paire pour sortir.
		char someByte = 0;
		(void) ::send(
					mPublicPairMember,
					&someByte,
...
TDCLSocket::TSocketPipe::Read( void* outBuffer, KUInt32* ioCount )
{
	...
	while( toRead > 0 )
	{
		...
		{
			// Erreur.
			// Mise ˆ jour du nombre d'octets lus.
			*ioCount -= toRead;
...
// MacOS X n'est pas conforme POSIX
			...
			// On vŽrifie que le dŽlai n'est pas dŽpassŽ.
...
TDCLSocket::TSocketPipe::Write( const void* inBuffer, KUInt32* ioCount )
{
	...
	while( toWrite > 0 )
	{
		...
		{
			// Erreur.
			// Mise ˆ jour du nombre d'octets Žcrits.
			*ioCount -= toWrite;
...
TDCLSocket::TSocketPipe::BytesAvailable( void )
{
	...
	// DŽlai de temporisation ˆ 0 pour savoir si on peut lire maintenant.
	int readySockets =
...
TDCLSocket::TSocketPipe::DoDisconnect( void ) 
{
	...
	(void) ((TDCLSocket*) GetCommLayer())->DoClose(
			...
			// Ceci fera Žchouer select dans le futur.
